...
//		$DB->connectDevelopment();
	...
	//多重なので、股並列
	if(count($pushToken) > 1000){

		$pushTokens = array_chunk($pushToken, 1000);
	...
	try{
		for($i=0;$i<count($pushToken);$i++){
			...
			$message->setCustomProperty('kakusu',1);
			$message->setCustomProperty('mail',$mail);
			$message->setSound("bingbong.aiff");
			$message->setCategoryProperty("INVITE_CATEGORY");
